...
function FallbackAnimal({ animal, callouts }) {
  const groupRef = useRef()
  ...
  return (
    <group ref={groupRef} position={[0, isDog ? -0.2 : -0.1, 0]}>
      <mesh position={[0, 0.5, 0]}>
        <sphereGeometry args={[isDog ? 0.35 : 0.3, 16, 16]} />
        <meshBasicMaterial color="#F4F0E6" wireframe transparent opacity={0.9} />
      </mesh>
      <mesh position={[0, 0.15, isDog ? 0.15 : 0.1]} rotation={[Math.PI / 2, 0, 0]}>
        <capsuleGeometry args={[isDog ? 0.22 : 0.18, isDog ? 0.7 : 0.55, 8, 16]} />
        <meshBasicMaterial color="#F4F0E6" wireframe transparent opacity={0.9} />
      </mesh>
      {callouts.map((callout, index) => (
        <Html key={callout.title} position={callout.position} center distanceFactor={6}>
          <div className="rounded-lg border border-pif-cream/40 bg-pif-dark/90 px-3 py-2 text-pif-cream/80">
            <p className="text-[9px] font-bold tracking-widest">{callout.title}</p>
            <p className="mt-1 font-mono text-sm font-semibold">{callout.stat}</p>
            <p className="text-[10px] text-pif-cream/70">{callout.sub}</p>
          </div>
...
function Scene({ animal }) {
  ...
  return (
    <>
      ...
      <ErrorBoundary
        fallback={<FallbackAnimal animal={animal} callouts={CALLOUTS[animal]} />}
      >
        <Suspense fallback={<FallbackAnimal animal={animal} callouts={CALLOUTS[animal]} />}>
          <WireframeModel
            url={url}
            scale={isDog ? 0.9 : 0.7}
...
export default function WireframeAnimals() {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-pif-cream/40 bg-pif-deep">
      <div className="flex flex-col gap-3 border-b border-pif-cream/20 px-4 py-3 sm:flex-row sm:items-center sm:justify-between md:px-6">
        <div>
      ...
      <div className="relative h-[360px] bg-pif-dark md:h-[480px]">
        <Canvas camera={{ position: [0, 1, 4], fov: 45 }} fallback={<div className="flex h-full items-center justify-center text-sm text-pif-cream/60">Loading 3D model...</div>}>
          <Scene animal={animal} />
        </Canvas>
